(function () {
  ...
  function buildAlumniSpotlightStoryHtml(st) {
    if (!st) return "";
  ...
  function renderHomePage() {
    ...
    if (hero && h.hero) {
      ...
      hero.innerHTML =
        ...
        '<div class="grid gap-3 lg:grid-cols-12 lg:items-stretch lg:gap-x-6 lg:gap-y-3">' +
        ...
        (fr.amount
          ...
          : "") +
        "</div>" +
        '<div class="mt-2 flex shrink-0 items-center gap-2 text-[11px] text-slate-600 sm:mt-3 sm:text-xs">' +
        '<span class="flex h-6 w-6 items-center justify-center rounded-full bg-slate-200 text-[9px] font-bold text-slate-500 sm:h-7 sm:w-7 sm:text-[10px]">' +
        esc(crest) +
        "</span><span>" +
        esc(fr.footerLine || "") +
    ...
    if (als && h.alumniSpotlight) {
      ...
      var alumniStories = Array.isArray(al.stories)
        ...
        : al.story
          ? [al.story]
          : [];
      var firstStory = alumniStories[0];
      var moreStories = alumniStories.slice(1);
      var featuredRow = "";
      if (firstStory) {
        featuredRow =
    ...
    return (
      ...
      (web3formsEnabled()
        ? '<form class="inquiry-form mt-6 grid gap-4 sm:grid-cols-2" action="' +
          ...
          '<div class="sm:col-span-2 pt-1"><button type="submit" class="w-full rounded-full bg-mes-accent px-8 py-3 font-semibold text-mes-primaryDark hover:bg-mes-accentLight">Submit inquiry</button></div>' +
          "</form>"
    ...
    el.innerHTML =
      ...
      (web3formsEnabled()
        ? '<form class="alumni-form mt-6 grid gap-4 sm:grid-cols-2" action="' +
          ...
          '<div class="sm:col-span-2 pt-1"><button type="submit" class="w-full rounded-full bg-mes-primary px-8 py-3 font-semibold text-white transition hover:bg-mes-primaryDark">Submit registration</button></div>' +
          "</form>"
  ...
  function buildContactFormHtml() {
    ...
    return (
      '<form id="contact-form" class="contact-form mt-3 flex flex-col gap-2.5" action="' +
      esc(WEB3_FORMS_ACTION) +
      '" method="POST">' +
      web3FormHiddenFields("Dr. Gadagkar High School — Website contact") +
      '<div><label class="block text-sm font-medium" for="cf-name">Name</label><input id="cf-name" name="name" required class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-1.5"/></div>' +
      '<div><label class="block text-sm font-medium" for="cf-email">Email</label><input id="cf-email" name="email" type="email" required class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-1.5"/></div>' +
      '<div class="grid gap-2.5 sm:grid-cols-2">' +
      '<div><label class="block text-sm font-medium" for="cf-phone">Phone</label><input id="cf-phone" name="phone" type="tel" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-1.5"/></div>' +
      '<div><label class="block text-sm font-medium" for="cf-subject">Subject</label><input id="cf-subject" name="topic" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-1.5"/></div>' +
      "</div>" +
      '<div><label class="block text-sm font-medium" for="cf-msg">Message</label><textarea id="cf-msg" name="message" rows="3" required class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-1.5"></textarea></div>' +
      '<div class="pt-0.5"><button type="submit" class="w-full rounded-full bg-mes-primary px-8 py-2 font-semibold text-white hover:bg-mes-primaryDark">Send message</button></div></form>'
    );
  }
